...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--run_dir',
                        help='Path to run directory (best validation snapshot is selected)')
    parser.add_argument('-w', '--weights', default='.',
                        help='Path to HDF5 weights file (ignored if specified with -r)')
    parser.add_argument('-o', '--out', default='out/',
                        help='Where to save predictions (relative path if specified with -r)')
    parser.add_argument('-a', '--arch', type=str, default='normal',
                        help='The network architecture ([normal])')
